...
class Config:
    ...
    train_ner_path = './data/longformer_train_ner.csv'
    ...
    model_dir = os.path.join(base_dir, f'model/')
    output_dir = os.path.join(base_dir, f'save/')
    is_debug = False
...
def agg_essays(train_flg):
    ...
    for f in tqdm(list(os.listdir(f'{Config.data_dir}/{folder}'))):
        ...
        texts.append(open(f'{Config.data_dir}/{folder}/' + f, 'r', encoding='utf-8').read())
...
def ner(df_texts, df_train):
    ...
    for _,  row in tqdm(df_texts.iterrows(), total=len(df_texts)):
        ...
        for _, row2 in df_train[df_train['id'] == row['id']].iterrows():
            ...
            list_ix = [int(x) for x in row2['new_predictionstring'].split(' ')]
...
def preprocess(df_train=None):
    ...
    df_texts = agg_essays(train_flg)        # get df : id, text, text_split

    if train_flg:
        df_texts = ner(df_texts, df_train)
    return df_texts

if not os.path.exists(Config.train_ner_path):
    alltrain_texts = preprocess(df_alltrain)
    alltrain_texts.to_csv(Config.train_ner_path, index=False, encoding='utf-8')
else:
    alltrain_texts = pd.read_csv(Config.train_ner_path, dtype=object)
alltrain_texts['entities'] = alltrain_texts['entities'].apply(ast.literal_eval)
alltrain_texts['text_split'] = alltrain_texts['text_split'].apply(ast.literal_eval)
        ...
        if self.has_labels:
            ...
            for word_idx in word_ids:
                if word_idx is None:
                    ...
                    if Config.label_subtokens:      # 如果一个单词被分为几块，则都用相同的 label
                        labels_ids.append(LABELS_TO_IDS[word_labels[word_idx]])
                    else:
                        labels_ids.append(IGNORE_INDEX)
...
def calc_overlap(row):
    ...
    len_gt = len(set_gt)
    intersection = len(set_gt.intersection(set_pred))       # 利用pandas.index.intersection，求2个列表关于索引列的交集
...
def score_feedback_comp(pred_df, gt_df):
    ...
    joined = pred_df.merge(                 # 将 (left_on)pred_df 的 class 与 (right_on)gt_df 的 discourse_type 相同的行合并起来
        gt_df,
        left_on=['id', 'class'],
        right_on=['id', 'discourse_type'],
        how='outer',                        # 合并采用并集的方式，不相同的行用 NaN 填充，‘inner’ 表示交集
        suffixes=['_pred', '_gt']           # 合并后，两个 df 列名相同的 key ，添加后缀做区分
    )
    joined['new_predictionstring_gt'] = joined['new_predictionstring_gt'].fillna(' ')       # ground truth      用 ‘ ’ 替代 NaN
    joined['new_predictionstring_pred'] = joined['new_predictionstring_pred'].fillna(' ')   # prediction
    ...
    tp_pred_ids = joined.query('potential_TP').sort_values('max_overlap', ascending=False) \
        .groupby(['id', 'new_predictionstring_gt']).first()['pred_id'].values                   ##### first() 分组的第一组 'id'
        # groupby 以 'id' 和 'new_predictionstring_gt'作为纵轴（index）
        # 找到 potential_TP 为 True 的 pred_id              matched predictions
    fp_pred_ids = [p for p in joined['pred_id'].unique() if p not in tp_pred_ids]      # potential_TP 为 False    unmatched predictions
    matched_gt_ids = joined.query('potential_TP')['gt_id'].unique()                     # matched ground truths
    unmatched_gt_ids = [c for c in joined['gt_id'].unique() if c not in matched_gt_ids] # unmatched ground truths
...
def inference(model, dl, criterion, valid_flg):
    ...
    for batch_idx, batch in enumerate(stream, start=1):
        ...
        logits = active_logits(raw_logits, word_ids)        # mask
        sf_logits = torch.softmax(logits, dim=-1)           # mask 之后的 softmax logits
        sf_raw_logits = torch.softmax(raw_logits, dim=-1)   # 没有 mask 的 softmax logits
        if valid_flg:
...
def preds_class_prob(all_logits, dl):
    ...
    for batch_idx, batch in enumerate(stream, start=0):
        for minibatch_idx in range(Config.valid_batch_size):
            ...
            pred_class_labels = [IDS_TO_LABELS[i] for i in pred_class_id]       # label_ids 转换成 label
            prev_word_idx = -1
            for idx, word_idx in enumerate(word_ids):
                if word_idx == -1:
                    ...
                    predictions_prob.append(pred_score[idx])        # 概率值
                    prev_word_idx = word_idx
            final_predictions.append(predictions)               # 所有预测的 label
            final_predictions_score.append(predictions_prob)    # 所有 label 的概率
    ...
    for i in range(len(df)):
        ...
        while j < len(pred):
            ...
            if cls != 'O' and cls != '':
                ...
                if end - j > MIN_THRESH[cls] and avg_score > PROB_THRESH[cls]:          # 如果小于概率 ??
                    final_preds.append((idx, cls.replace('I-', ''), ' '.join(map(str, list(range(j, end))))))
                    # 每一个 id 的文件，预测的 label 都是按顺序排列的
            j = end
...
def train_fn(model, dl_train, optimizer, epoch, criterion):
    ...
    scaler = GradScaler()           # 自动混合精度 https://www.cnblogs.com/jimchen1218/p/14315008.html
    ...
    for batch_idx, batch in enumerate(stream, start=1):
        ...
        word_ids = batch['word_ids'].to(device, dtype=torch.long)
        optimizer.zero_grad()
        with autocast():            # https://blog.csdn.net/qq_36955294/article/details/115186228
            raw_logits = model(input_ids=ids, mask=mask)

        logits = active_logits(raw_logits, word_ids)        # (-1, Config.num_labels)   mask 之后的 logits
        labels = active_labels(raw_labels)                  # (-1)  mask 之后的 labels
        sf_logits = torch.softmax(logits, dim=-1)
        preds, preds_prob = active_preds_prob(sf_logits)    # preds 得到预测类别
        train_accuracy += accuracy_score(labels.cpu().numpy(), preds.cpu().numpy())     # accuracy_score() 计算 acc
...
def valid_fn(model, df_val, df_val_eval, dl_val, epoch, criterion):
    oof, valid_loss, valid_acc = get_preds_onefold(model, df_val, dl_val, criterion, valid_flg=True)       # oof 表示预测输出
    f1score =[]
...
for i_fold in range(Config.n_fold):
    ...
    df_train = alltrain_texts[alltrain_texts['fold'] != i_fold].reset_index(drop=True)      # 训练集 alltrain_texts: longformer_train_ner.csv
    ds_train = FeedbackPrizeDataset(df_train, tokenizer, Config.max_length, True)
    df_val = alltrain_texts[alltrain_texts['fold'] == i_fold].reset_index(drop=True)        # 一个 fold 的验证集
    val_idlist = df_val['id'].unique().tolist()                 # 验证集的文件名列表
    df_val_eval = df_alltrain.query('id==@val_idlist').reset_index(drop=True)               # 验证集；df_alltrain: corrected_train.csv
    ds_val = FeedbackPrizeDataset(df_val, tokenizer, Config.max_length, True)
    dl_train = DataLoader(ds_train, batch_size=Config.train_batch_size, shuffle=True, pin_memory=True)      # num_workers=2
    dl_val = DataLoader(ds_val, batch_size=Config.valid_batch_size, shuffle=False, pin_memory=True)
    ...
    for epoch in range(1, Config.n_epoch + 1):
        ...
        # model.load_state_dict(torch.load('./model/longformer_0fold.bin', map_location='cpu'))
        valid_loss, _oof = valid_fn(model, df_val, df_val_eval, dl_val, epoch, criterion)   # _oof 表示一个 fold 的验证集预测输出
        if valid_loss < best_val_loss:
            best_val_loss = valid_loss
            _oof_fold_best = _oof
            _oof_fold_best['fold'] = i_fold
            model_filename = f'{Config.model_dir}/{Config.model_savename}_{i_fold}fold.bin'
            torch.save(model.state_dict(), model_filename)
            print(f'{model_filename} saved')

    oof.to_csv(f'{Config.output_dir}/oof_{Config.name}_{i_fold}fold.csv', index=False)
...
print(f'overall cv score: {oof_score(df_train, oof)}')      # df_train 真实值， oof 整个训练集的预测值
